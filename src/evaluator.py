...
class Evaluator(object):
    ...
    def enc_dec_step(self, data_type, task, scores):
        ...
        assert task in ['prim_fwd', 'prim_bwd', 'prim_ibp', 'ode1', 'ode2', 'add_dataset']
        ...
        for (x1, len1), (x2, len2), nb_ops in iterator:
            ...
            alen = torch.arange(len2.max(), dtype=torch.long, device=params.my_device)
            pred_mask = alen[:, None] < len2[None] - 1  # do not predict anything given the last target word
            y = x2[1:].masked_select(pred_mask[:-1])
            assert len(y) == (len2 - 1).sum().item()

            # cuda
            x1, len1, x2, len2, y = to_cuda(params.my_device, x1, len1, x2, len2, y)
            ...
            t = torch.zeros_like(pred_mask, device=params.my_device)
