...
class CompressingBlock(nn.Module):
    ...
    def __init__(self, config: TransformerConfig, inp_size: int, inp_embd: int, out_size: int, out_embd: int):
        ...
        self.block = Block(dropout, inp_size, inp_embd * 4, inp_embd, inp_embd, n_head, inp_embd // n_head)

        self.ln1 = nn.LayerNorm(inp_size * inp_embd)
        self.ffwd = FeedForward(inp_size * inp_embd, inp_size * inp_embd * 4, out_size * out_embd, dropout)
    ...
    def forward(self, x):
        ...
        x = self.ffwd(self.ln1(x))

        x = x.reshape(b, self.out_size, self.out_embd)
