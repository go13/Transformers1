...
from t3_karpathy.flash_karpathy_transformer import FlashKarpathyRunner
...
config = TransformerConfig(precision=torch.bfloat16, batch_size=32, block_size=512, n_embed=64, n_head=4, n_layer=10)
dataloader = GptNanoDataloader(config)
runner = FlashKarpathyRunner(config, dataloader)
